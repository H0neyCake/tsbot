...
function CheckForRules( serverConnectionHandlerID, clientID, kostil )
	...
	if ( channelID == chanelAltisId or channelID == chanelStratisId or channelID == chanelUnknownId ) and channelError == 0 then
		...
		if (tonumber(inputStatus) == 1 and inputError == 0) or (tonumber(_inputStatus) == 0 and _inputError == 0) then
			...
			ts3.requestClientKickFromChannel(serverConnectionHandlerID, clientID, "Включите ваш микрофон!")
			ts3.requestSendPrivateTextMsg(serverConnectionHandlerID, string.format("Уважаемый %s. Вы нарушили правила использования TeamSpeak! Во время пребывания на сервере ЗАПРЕЩЕНО выключать микрофон!", name), clientID)
		...
		if (tonumber(outputStatus) == 1 and outputError == 0) or (tonumber(_outputStatus) == 0 and _outputError == 0) then
			...
			ts3.requestClientKickFromChannel(serverConnectionHandlerID, clientID, "Включите ваши динамики!")
			ts3.requestSendPrivateTextMsg(serverConnectionHandlerID, string.format("Уважаемый %s. Вы нарушили правила использования TeamSpeak! Во время пребывания на сервере ЗАПРЕЩЕНО выключать звук!", name), clientID)
			ts3.requestSendPrivateTextMsg(serverConnectionHandlerID, "Для входа в игровую комнату зайдите в раздел Плагинов/Аддонов TeamSpeak, и нажмите кнопку <Обновить>.", clientID)
		end

		if tonumber(awayStatus) == 1 and awayError == 0 then
			SaveToLog(string.format("[CLIENT_AWAY] Kick: %s", name), "Kick")
			ts3.requestClientKickFromChannel(serverConnectionHandlerID, clientID, "Выключите режим AFK!")
			ts3.requestSendPrivateTextMsg(serverConnectionHandlerID, string.format("Уважаемый %s. Вы нарушили правила использования TeamSpeak! Во время пребывания на сервере ЗАПРЕЩЕНО включать режим AFK!", name), clientID)
			ts3.requestSendPrivateTextMsg(serverConnectionHandlerID, "Для входа в игровую комнату зайдите в раздел Плагинов/Аддонов TeamSpeak, и нажмите кнопку <Обновить>.", clientID)
		end
...
function NyllCoreEventHandler.onClientMoveEvent( serverConnectionHandlerID, clientID, oldChannelID, newChannelID )
	...
	if overWolfStatus and overWolfError == 0 then
		...
		if string.find(overWolfStatus, "Overwolf=1") then
			...
			ts3.requestSendPrivateTextMsg(serverConnectionHandlerID, string.format("Уважаемый %s. Вы нарушили правила использования TeamSpeak! Во время пребывания в нашем TeamSpeak ЗАПРЕЩЕНО включать программу Overwolf!", name), clientID)
	...
	if clientID and newChannelID then
		if tContains(gameRoomID, tonumber(newChannelID)) then
			...
			ts3.requestSendPrivateTextMsg(serverConnectionHandlerID, "У вас нет доступа к данной комнате!", clientID)
		end
	end
end
